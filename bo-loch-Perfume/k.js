...
// Hàm kiểm tra sản phẩm có kieu == dior
function checkProductsByKieu() {
    const storedProducts = JSON.parse(localStorage.getItem('cardList')) || []; // Lấy dữ liệu từ local storage
    const filteredProducts = storedProducts.filter(product => product.kieu === 'dior'); // Lọc sản phẩm

    if (filteredProducts.length > 0) {
        console.log('Sản phẩm có kieu == dior:', filteredProducts);
    } else {
        console.log('Không tìm thấy sản phẩm nào có kieu == dior.');
    }
}
